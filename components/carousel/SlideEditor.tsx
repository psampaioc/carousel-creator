...
export function SlideEditor({ slide }: { slide: Slide }) {
  ...
  return <form className="slide-editor" onSubmit={async (event) => {
    ...
    {slide.kind === "event" && slide.images.length ? <fieldset className="image-picker"><legend>Drive images · provenance order</legend>{slide.images.map((image) => <label key={image._id} className={imageCandidateId === image._id ? "is-selected" : ""}><input type="radio" name={`image-${slide._id}`} checked={imageCandidateId === image._id} onChange={() => setImageCandidateId(image._id)} /><span className="image-thumb" role="img" aria-label={`${image.provenance} image preview`} style={{ backgroundImage: `url(https://drive.google.com/thumbnail?id=${encodeURIComponent(image.driveFileId)}&sz=w240)` }} /><span><strong>{image.provenance === "generated" ? "AI-generated" : image.provenance}</strong><small>{image.sourceUrl}</small></span></label>)}</fieldset> : null}
